#include "awtk.h"
#include "navigator.h"
extern ret_t demo1_init(widget_t* win, void* ctx);
extern ret_t demo_init(widget_t* win, void* ctx);
extern ret_t demo3_init(widget_t* win, void* ctx);
extern ret_t demo2_init(widget_t* win, void* ctx);
#ifndef WITH_MVVM

static ret_t navigator_window_init(const char* name, widget_t* win, void* ctx) {
    if (tk_str_eq(name, "demo1")) {
      return demo1_init(win, ctx);
    } else if (tk_str_eq(name, "demo")) {
      return demo_init(win, ctx);
    } else if (tk_str_eq(name, "demo3")) {
      return demo3_init(win, ctx);
    } else if (tk_str_eq(name, "demo2")) {
      return demo2_init(win, ctx);
    } 
return RET_OK;
}
